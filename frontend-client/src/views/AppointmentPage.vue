    ...
    <b-container>
        <b-modal id="my-modal" hide-footer title="Start appointment">
            ...
            <b-button
                ...
                :disabled="!timeValid"
                block
                @click="patientNotShowup"
                >Patient did not show up!</b-button
            >
            <b-button
                class="mt-2"
                variant="outline-hub"
...
export default {
    ...
    data: function() {
        return {
            selectedOption: "",
            options: [
                { text: "First Name", value: "patient.name" },
    ...
    methods: {
        patientProfileView(item) {
            if (this.user.type == "Dermatologist") {
                this.$router.push({
                    ...
                    params: { passedId: item.patient.id, check: "derm" },
                });
            } else {
                this.$router.push({
                    name: "PatientProfileView",
                    params: { passedId: item.patient.id, check: "pharm" },
                });
            }
        },
        getAllAppointments: function() {
            if (this.user.type == "Dermatologist") {
                this.$http
                    .get(
                        `http://localhost:8081/dermatologist-appointment/all-derm-todo?page=${this
                    ...
                    .then((response) => {
                        ...
                        this.appointments = response.data.map(
                            (currentEvent) => ({
                                ...
                                start:
                                    currentEvent.date.substring(0, 10) +
                                    " " +
                                    currentEvent.time,
                ...
                this.$http
                    .get(
                        `http://localhost:8081/pharmacist-appointment/all-pharm-todo?page=${this
                            .currentPage - 1}&size=5`,
                        {
                            params: {
                    ...
                    .then((response) => {
                        ...
                        this.appointments = response.data.map(
                            (currentEvent) => ({
                                ...
                                last_name: currentEvent.patient.surname,
                                drugstore:
                                    currentEvent.pharmacist.drugstore.name,
                                start:
                                    currentEvent.date.substring(0, 10) +
                                    " " +
                                    currentEvent.time,
        ...
        getRows() {
            if (this.user.type == "Dermatologist") {
                this.$http
                    .get(
                        `http://localhost:8081/dermatologist-appointment/all-derm-todo-length`,
                        {
                            params: {
                                dermatologistId: this.user.id,
                            },
                ...
                this.$http
                    .get(
                        `http://localhost:8081/pharmacist-appointment/all-pharm-todo-length`,
                        {
                            params: {
                                pharmacistId: this.user.id,
                            },
        ...
        patientNotShowup: function() {
            ...
            if (this.user.type == "Dermatologist") {
                this.$http
                    .get(
                        "http://localhost:8081/dermatologist-appointment/end-appointment",
                        {
                            params: {
                ...
                this.$http
                    .get(
                        "http://localhost:8081/pharmacist-appointment/end-appointment",
                        {
                            params: {
        ...
        startApp: function() {
            if (this.user.type == "Dermatologist") {
                ...
                this.$router.push({
                    ...
                    params: { passedId: this.selected.id },
                });
            } else {
                this.$toastr.s("Appointment started" + this.selected.id);
                this.$root.$emit("bv::hide::modal", "my-modal");
