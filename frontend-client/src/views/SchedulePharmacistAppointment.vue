<template>
    <b-container>
        <b-card>
            <h1 v-if="(employees.length == 0) & (drugstores.length == 0)">
                Choose a date and time for your appointment
            </h1>
            <h1 v-if="(employees.length == 0) & (drugstores.length != 0)">
                Choose a drugstore for your appointment
            </h1>
            <h1 v-if="(employees.length != 0) & (drugstores.length != 0)">
                Choose a pharmacist for your appointment
            </h1>

            <b-form
                v-if="(employees.length == 0) & (drugstores.length == 0)"
                @submit="showDrugstores"
            >
                <b-form-group
                    label="Pharmacist appointment date"
                ...
                <b-form-group
                    label="Pharmacist appointment time"
            ...
            <br />
            <h1 v-if="(searched == 1) & (drugstores.length == 0)">
                Sorry, there are no available pharmacists at this time.
            </h1>
            ...
            <b-table
                ...
                :per-page="4"
                :items="drugstores"
                :fields="fieldsDrugstores"
                @row-clicked="getPharmacists"
            >
            </b-table>
            <b-pagination
                v-model="currentPage"
                v-if="(employees.length == 0) & (drugstores.length != 0)"
                :total-rows="drugstores.length"
                :per-page="4"
...
export default {
    ...
    methods: {
        saveAppointment: function(data) {
            ...
            this.$http
                .post(
                    "http://localhost:8081/pharmacist-appointment/with-patient",
                    JSON.parse(JSON.stringify(this.saveData))
                )
                .then((response) => {
        ...
        getPharmacists: function(data) {
            this.$http
                .get(
                    "http://localhost:8081/pharmacist-appointment/get-pharmacists",
                    {
                        params: {
                            drugstoreId: data.id,
        ...
        showDrugstores: function(event) {
            ...
            if (this.inputValues.date == "" || this.inputValues.time == "")
                return;
            this.searched = 1;
            this.$http
                .get(
                    "http://localhost:8081/pharmacist-appointment/get-drugstores",
                    {
                        params: {
                            pharmacistAppointmentDate: this.inputValues.date,
                            pharmacistAppointmentTime: this.inputValues.time,
                ...
                .then((response) => {
                    this.drugstores = response.data.map((drugstoreDto) => ({
                        ...
                        pharmacistAppointmentPrice:
                            drugstoreDto.pharmacistAppointmentPrice,
                    }));
                })
                .catch((error) => console.log(error));
        },
